...
(function() {
  const render = () => {
    ...
    if (!filmCatalog?.length) return;
    const trending = filmCatalog.filter(f => f.isTrending);
    grid.innerHTML = trending.map(f => `<a href="genre.html?slug=${f.slug}" class="trending-card"><div class="card-image-wrapper"><img src="${f.poster}" alt="${f.title}"/></div><p class="trending-card-title">${f.title}</p></a>`).join("");
  };
